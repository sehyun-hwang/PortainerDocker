...
nginx)
	# docker compose start pgadmin portainer registry-browser redisinsight
	NETWORK=$(docker compose config | awk '/networks:/ { getline; if ($0 ~ /default:$/) { getline; print $2 }}')

	docker pod ls -f=status=degraded -q | grep nginx-pod && docker pod rm nginx-pod
	docker pod create --name nginx-pod --net $NETWORK \
		-p 80:80 -p 443:443/tcp -p 443:443/udp || true
